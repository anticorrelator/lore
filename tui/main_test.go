...
// TestArchiveOpensConfirmModal verifies pressing A on a work item in the list
// panel opens the archive confirm modal.
func TestArchiveOpensConfirmModal(t *testing.T) {
	m := minimalModel(stateWork, []work.WorkItem{{Slug: "ok-1", Title: "OK", Status: "active"}}, nil)
	m.width, m.height = 120, 40
	m.focusedPanel = panelLeft
	nm, _ := updateModel(t, m, press('A'))
	if nm.confirmAction != "archive" {
		t.Errorf("A should open the archive modal, got %q", nm.confirmAction)
	}
}
