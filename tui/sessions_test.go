...
func TestTabIndicatorIdentityRendering(t *testing.T) {
	...
	baseW := lipgloss.Width(strings.TrimRight(stripANSI(renderTabIndicator(stateWork, 1, 2, 4, 0, 5, 200, "")), " "))
	for _, width := range []int{60, 80, 120, 200} {
		out := stripANSI(renderTabIndicator(stateWork, 1, 2, 4, 0, 5, width, identity))
		wantW := width
		if baseW > width {
			wantW = baseW
		}
