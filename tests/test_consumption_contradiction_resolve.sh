...
# find-correction-targets reads concordance vectors, which `pk_cli index` does not
# build — populate them the same way `lore rebuild` does. A second entry is
# required: with a one-document corpus every term's IDF is log(1/1)=0, so no
# vector would be built for the fixture entry.
cat > "$KDIR/architecture/decoy-naming-rule.md" <<'EOF'
# Database naming

Tables use snake_case plural nouns. Columns use snake_case singular.

<!-- learned: 2026-05-09 | confidence: high | source: test | related_files: db/schema.sql | scale: subsystem | status: current -->
EOF
LORE_KNOWLEDGE_DIR="$KDIR" python3 "$PK" index "$KDIR" --force >/dev/null
PYTHONPATH="$SCRIPTS_DIR" python3 -c '
import sys, os
from pk_concordance import Concordance
Concordance(os.path.join(sys.argv[1], ".pk_search.db")).build_vectors()
' "$KDIR" >/dev/null
...
assert_eq "find json first path is fixture" "$(echo "$FIND_JSON" | jq -r '.targets[0].path')" "$ENTRY"
