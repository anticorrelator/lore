...
| **Verified cleanup gates completion** | A writer is not `done` until reconciliation is full and cleanup proves path, Git worktree registry, and branch/ref disposition. Crash sweeping records recovery evidence before removal; `cleanup_blocked` is retry-only. Acceptance itself is anchored: `freeze-integrated` pins `refs/lore/accepted/*` at the integrated commit and stream tip before recording the acceptance, so accepted content stays reachable after the sweep deletes the temporary branch, and `status` audits each recorded anchor. | Reconciliation aggregate, manager archive proof, acceptance refs pinned by `coordinate-reconcile.py`, coordinated `impl-close` precondition. | A process terminal or successful merge cannot launder leaked worktrees or destroyed abandoned changes into closure — and cleanup can delete the branch without ever stranding the accepted commits it anchored. |
