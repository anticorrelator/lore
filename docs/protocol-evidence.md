...
A packet is handed on synthesized or with its waiver recorded; an assembled row is a candidate set. `lore packet synthesize <id> --by <who> [--drop <path> "<reason>"]... [--add <path> "<reason>"]... [--spec <file.json>]` appends a superseding row under the same `packet_id` through `packet-append.sh` at `delivery_stage: synthesized`, carrying a `synthesis` object (`by`, `synthesized_at`, `kept`, `dropped[{path, reason}]`, `added[{path, reason}]`). Assembly records the rendered block of every delivered entry (`delivered_entries[].rendered`); synthesis locates every recorded block in delivery order and removes the dropped entry's own span, so a heading inside an entry's body cannot end the removal early and a literal copy of the same bytes embedded in a neighbour rendered earlier is left alone — a candidate whose assembly did not record blocks, or whose blocks cannot be placed in delivery order, cannot be dropped from and says so. Added entries are rendered from their files under `### Added by synthesis`; when anything was dropped, a `## Left out by synthesis` list (title, path, reason) closes the content. `delivered_entries`, `entries_per_scale` and `trust_snapshot_hash` are recomputed over the kept and added entries; an added entry's scale, status and confidence come from its own footer. Kept is every delivered path not dropped; only delivered paths can be dropped; only knowledge entries (a `.md` file in a category directory inside the store, by canonical path) not already delivered can be added; a row that is not `assembled` cannot be synthesized again without a re-pull, and the check and the append are sequential, not atomic. `packet-append.sh` refuses a row whose identity (`schema_version`, `packet_scope`, `work_item`, `task_id`, `revision_id`, `dispatch_attempt_id`, `source_head`, `recipient_role`, `session_id`) differs from any prior row under the same id, so a chain never changes what it is about. `position-bind` refuses to prepare a dispatch against a packet whose latest row is not `synthesized` unless it carries `synthesis_waiver: {by, reason}`; `spec-open` records that waiver on the investigator packets it builds itself (the default-bindings branch of its full wave) — a caller that builds a packet and supplies its own bindings synthesizes first. Pointer-only surfaces (`--packet` on a session request, the short guidance pointer) resolve existence and do not enforce the rule. `lore packet show` returns the latest row for an id and names the synthesis, the waiver, or their absence; `work-evidence` folds the chain to one `packet_summary` entry per id with the stage, the synthesis counts and `superseded_rows`; the packet assessor owes one verdict per id. Nothing here scores the judgment; retro reads synthesis rows beside retrieval-miss telemetry.
