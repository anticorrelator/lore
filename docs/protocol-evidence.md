...
A packet is handed on synthesized or with its waiver recorded; an assembled row is a candidate set. `lore packet synthesize <id> --by <who> [--drop <path> "<reason>"]... [--add <path> "<reason>"]...` appends a superseding row under the same `packet_id` through `packet-append.sh` at `delivery_stage: synthesized`, with a `synthesis` object (`by`, `synthesized_at`, `kept`, `dropped[{path, reason}]`, `added[{path, reason}]`), content re-rendered without the dropped blocks and with a `## Left out by synthesis` list, and `delivered_entries`, `entries_per_scale` and `trust_snapshot_hash` recomputed. Kept is every delivered path not dropped; only delivered paths can be dropped; only store entries not already delivered can be added; a synthesized or delivered row cannot be synthesized again without a re-pull. `position-bind` refuses to prepare a dispatch against an assembled packet that carries no `synthesis_waiver: {by, reason}`; `spec-open` records that waiver for its full investigator wave, where assembly and dispatch happen in one verb. `lore packet show` returns the latest row for an id and names the synthesis, the waiver, or their absence. Nothing here scores the judgment; retro reads synthesis rows beside retrieval-miss telemetry.
