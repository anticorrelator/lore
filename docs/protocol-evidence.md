...
This document describes how protocol evidence is read from a work item, and what records future producers must write so the same reader can show them beside today's evidence. The reader is implemented, and so are the revision, packet, and review producers. The producer marked **planned** is not. Its section defines the target record contract a later task implements. Interface names given for the planned producer are the verbs the design names; its arguments and flags beyond those are not yet chosen and are not listed here.
