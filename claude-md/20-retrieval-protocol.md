...
`preferences/` holds the project's working-style entries — how things are done here — each scoped to an activity: PR bodies, review comments, work-item scoping, model routing, protocol prose. They record accumulated taste rather than claims about the code.

`/spec`, `/implement`, and `/coordinate` deliver them to you. Outside those, run `lore search --preferences "<activity>"` before anything another person will read (PR body, comment, commit message, docs) and before a judgment call that may already be settled (deferral, routing, ceremony). It searches only `preferences/`, takes no scale flag, and ranks by content and scope path; plain `lore search` buries preferences under descriptive entries.
