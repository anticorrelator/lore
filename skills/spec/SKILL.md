...
4. **Land the report before anything reads it** (full branch, compiled path). The report is evidence of record only as the file the sole writer lands at the destination assigned at dispatch; the message, tool result, or transcript that carried it is transport. Land it verbatim, under the report id from the directive's bindings:
   ...
   On a directive that `open` left `pending-execution-root`, the lead holds no reference yet: the host published the payload when it launched the session. Read that publication independently, from the context file retained at enqueue:

   ```bash
   python3 ~/.lore/scripts/position-bind.py session-reference --kdir "$KNOWLEDGE_DIR" < "$CONTEXT_FILE"
   ```

   It exits nonzero while no manifest exists for the attempt, which is what an unlaunched or refused session looks like from here and is a different state from a landed report. On success it returns `reference` (the same six manifest, payload, and native fields a prepared directive carries), `completion_input`, the actual `bindings` including the host's root, `producer`, `publication_state: prepared`, and `delivery_proven: false`. Hold `reference` and `completion_input` for the directive in the places `open` left `null`. The command checks the published bundle against the admitted preparation (bindings, descriptor, guidance, wrapper text, reconstructed payload, packet, physical root, and the published `launch.json` against the retained activation snapshot rather than a fresh rendering, so a renderer that has moved since admission cannot change what the reference says) and reads nothing from the report. A report that arrives while the command still exits nonzero was not produced by a launch the binder published for this attempt; land it as the record of what came back and treat it under the disagreement below. `delivery_proven: false` is literal: the reference establishes what was prepared, and only the landed report and the completion check say what came back.
