...
1. **Build the designer's packet** — session-scoped, one retrieval pass for this question at a declared scale:
   ...
   Do not pass `--task`. A task-bound packet is bound to the latest dispatch attempt recorded for that task, which is the worker's, and this consultation is its own attempt; the binder would refuse the collision. The packet therefore has no task or revision, and the bindings say so explicitly. The lead synthesizes this packet (`lore packet synthesize`) before this spawn, so the advisor receives the synthesized row.

2. **Assemble the bindings.** `work_item`; `task_id` and `revision_id` null, with the reason `consultation packet is session-scoped; the worker's task and revision are in the assignment`; `packet_id` and `packet_pointer` from step 1, naming the packet the lead synthesized before this spawn (`lore packet synthesize`) so the advisor receives the synthesized row; `dispatch_attempt_id` fresh (`consult-<uuid>`); `assignment` as a JSON object carrying the worker's request verbatim plus the worker's name, task id, and `Revision-id`; `report_id` `<advisor-name>-<consultation-id>` made filesystem-safe, `report_path` `$ITEM_DIR/worker-reports/<report-id>.md`; `execution_root` the requesting task's execution directory, because the question is about that code; `mode` `consultation`; `consultation_id`, `domain`; `reply_destination` `$ITEM_DIR/consultation-replies/<consultation-id>.md`. Each later request gets fresh attempt, report, and reply identities; a retained process is not permission to reuse an old request's manifest.
...
4. **Activate on the native surface.** On Claude Code:
   ...
   Check that `readiness.selection_name` is among the `subagent_type` values the live `Agent` tool offers; a file on disk is not that check. Then call `Agent` with `subagent_type`, `model`, and `prompt` from `tool_input`, plus `description`, `team_name` `impl-<slug>`, `name` `<advisor-name>`, and `mode`. On Codex, `native-input` without `--scope` returns the `spawn_agent` fields with `message`; add only the naming and fork fields the live tool requires, and keep the handle the tool returns, because that handle is what any later follow-up operation addresses. When the name is not offered, or the tool rejects the fields, the request is unanswered by a designer: record the boundary, answer inline as `handler: lead`, and do not select a generic agent, because the reply would then carry a version for a brief nobody read.
...
The designer's tools come from its definition and its harness — the compiled Claude definition has Read, Glob, Grep, and Bash and no messaging or task tool; a Codex child has whatever its harness supplies — so the reply route that works on every surface is the file: the designer writes its reply to `reply_destination` and returns the same text as its result where the surface returns one. Whichever arrives, the file is what you read. The reply is transport until landed:
...
4. Forward the reply body to the requesting worker through the follow-up operation the live surface exposes for that worker's retained handle — on Claude Code `SendMessage` to its name; elsewhere whatever Step 3.0's probe found — and the reply resumes it. Where the probe found no operation for that handle, the answer exists and is filed, but it was not delivered: record the delivery boundary beside the filing and in `notes.md`, and let the worker's task come back blocked on the unanswered domain rather than logging the consultation as if it had reached the worker.
...
A later request needs its own packet, bindings, and bound attempt (steps 1–3 above, with fresh identities), and then a delivery to the process that is already running. Before delivering, compare the new manifest's producer and framework, the resolved model, and the execution root with the recorded activation. When they match, deliver the new payload bytes unchanged through the follow-up operation the live surface exposes for the recorded handle: on Claude Code that is `SendMessage` to the designer's name; on another harness it is whatever operation the session's own tools offer for a spawned child's handle, which Step 3.0's operation-level probe established at dispatch. The adapter's `team_messaging` cell describes the adapter's channel and settles nothing about a live child; only the probe of the actual surface does. The designer answers at the new `reply_destination`, and collection runs as above. Log nothing as spawned, because nothing was.

Two situations end that path, and each is recorded rather than worked around. When guidance or the brief changed so the new descriptor's version differs from the activated one, the running process read a different brief than the new payload names, and sending it would attribute the reply to text it never read. The default is settled here so an unattended lead need not stop: send the activated designer its shutdown request and write its shutdown line with the old version, then activate a fresh designer from the new compilation for this request through the activation steps above, logging the spawn with the new version and keeping the same model resolution and pins. Two activations under one advisor name then appear in the log because the brief changed between them, and the rollup's grouping by version keeps them apart, which is the truthful record. When the probe finds no follow-up operation for the recorded handle, the first request was the only one that process can receive; answer later requests as `handler: lead`, note the boundary in `notes.md`, and do not spawn a replacement designer per request, because a second process under the same name and version would claim a continuity that did not exist.
...
The reply travels as a file, because that route works whatever tools your definition and harness gave you (the compiled Claude definition has Read, Glob, Grep, and Bash and no messaging tool). Write it to `position_dispatch.bindings.reply_destination` (create the directory if needed) and also return it as your final message. The lead reads it, lands it through the sole report writer, files the acknowledgment, and forwards it to the worker. Its first four lines are the headers the ledger joins on: `consultation-id: <from the assignment>`, `handler: agent`, `advisor_template_version: <position_dispatch.producer.template_version>`, `advisor-acknowledged: true`; then **Domain:**, **Guidance:**, **Key files:**, **Cautions:**. A reply missing a header cannot be joined to the worker's required domains.
