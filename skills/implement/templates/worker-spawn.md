...
5. **Launch by route.**
   ...
   - **Native, Claude Code** (`source_framework == target_framework == claude-code`). The compiled definition has to be a name the running session's `Agent` tool offers, and the binder owns the file:
     ...
     The compiled definition carries Read, Glob, Grep, Bash, Write, and Edit, and no task or messaging tools. So the lead owns the team-task lifecycle for this worker: set `owner` at launch, and the worker's report comes back as the call's result, which is transport. Land it with `lore coordinate report "$SLUG" --report-id "$REPORT_ID"`, run the completion check with `{"position_dispatch": <reference>, "lore_task_id": "<task-id>"}` on stdin (Step 4 §1), and only then mark the task completed. A required consultation arrives the same way: the worker returns its `## Consultation` request and stops; the lead's reply, sent to the worker's `name`, resumes it.

   - **Native, Codex** (`source_framework == target_framework == codex`). Run `native-input` without `--scope`; nothing is registered, because Codex takes the prepared text directly. `tool_input` carries the split model and effort keys plus `message` filled with the exact payload, and `readiness` is `{kind: native-tool-schema, tool: spawn_agent}`: the check owed is that the tool exposed in this session accepts those fields. Supplement only the caller fields its schema requires — a `task_name`, a non-inheriting fork setting — and pass the rest as returned. A model or effort the tool rejects is reported as rejected, not dropped.
   ...
   - **Session** (explicit selection only). The session binder publishes at launch through `session request --position`, so there is no `bind` call here and no wrapper member: write a position context file `{"bindings": <the assembled bindings with execution_root null and a reason, or the fixed placement's directory>, "descriptor": <descriptor object>, "guidance_file": "<path>", "class": "<mechanical|judgment-dense>" when set, "ceremony": "implement"}` and pass it to `agents/session-worker.md` as described in its section. The reference for this attempt is the manifest the host publishes under `position-dispatch/<attempt-id>/`; validate it with `position-bind.py validate` against the attempt, report id, and descriptor you minted before reading the report's headers against it.
...
Codex and OpenCode adapters report `team_messaging=none`, so a compiled worker on those harnesses cannot receive a mid-task reply; a task with required consultation domains on such a target is a capability boundary to record, not a reason to drop the requirement.
...
- **Compiled path.** The file is the position context from the compiled dispatch section: `bindings` (with `execution_root` null and a reason when the host allocates the tree, or the fixed placement's directory when the manager already allocated it), `descriptor`, `guidance_file`, `class` when the task has one, and `ceremony`. `session request --position worker` validates it at admission and the claiming host publishes the immutable payload after it knows the physical directory; a publication or activation failure returns an error before the process spawns. The session receives the compiled brief as its native definition or prompt, the identity envelope in its prompt, and the manifest path and digest in `LORE_POSITION_DISPATCH_MANIFEST` and `LORE_POSITION_DISPATCH_SHA256`. Write it to `$KDIR/_work/<work-item-slug>/worker-reports/<report-id>.context.json` and pass that path as `{{context_file}}`, with `{{dispatch_route}}` `compiled`, `{{framework}}`, `{{packet_id}}`, `{{report_file}}` (the bound `report_path`), and `{{producer_template_version}}`.
- **Legacy path.** The brief needs no guidance render of its own — `session request --type worker` renders the floor and prepends it at enqueue; the brief carries task content alone. (The chaperone's own Task prompt still gets its fresh rendering.) The brief is the same worker protocol content the default route resolves from `agents/worker.md` (task assignment with its composed brief + prior knowledge + evidence contract), adapted for session execution. Write it to `$KDIR/_work/<work-item-slug>/worker-reports/<derived-slug>.brief.md` and pass that path as `{{brief_file}}`, with `{{dispatch_route}}` `legacy`. The session is a standalone harness session, not a team subagent, so the brief's adaptations are:
  ...
  On the compiled path the same four facts hold and the compiled brief already states them: the report lands at the bound `report_path` through `lore coordinate report` before terminus, Tier 2 rows are self-appended with the task id from the envelope, there are no team tools, and terminus is explicit.
...
Task:
  ...
  prompt: |
    ...
    <contents of agents/session-worker.md with {{template}} variables resolved:
     ...
     {{context_file}}, {{framework}}, {{packet_id}}, {{report_file}}, and
     {{producer_template_version}} (compiled)>
```
...
Your definition has Read, Glob, Grep, Bash, Write, and Edit, and no task-list or messaging tool. The lead holds the team task for you. When your assignment declares required consultation domains, return the `## Consultation` request as your whole message and stop; the reply resumes you.
