...
3. **Name the wrapper and author its bytes.** The wrapper is the routing file whose text is delivered around the compiled brief, recorded apart from the brief so both stay attributable:
   ...
   The prefix is the identity header lines, in this order, one per line: `Packet-id:`, `Report-id:`, `Revision-id:`, `Dispatch-attempt-id:`. The lead synthesizes the packet `Packet-id:` names (`lore packet synthesize`) before this spawn, so the worker receives the synthesized row. The suffix is the dispatch note at the end of this file, filled in for this task. Neither may carry a guidance block; the binder refuses a second floor. The wrapper follows the text the worker actually reads around the brief, so it changes with the route: on the two native routes it is this file; on the chaperone route it is `agents/codex-worker.md` (`template_id` `implement/codex-worker`) with that file's chaperone note as the suffix; on the session route, fixed or ordinary, it is `agents/session-worker.md` (`template_id` `implement/session-worker`) with that file's session note as the suffix. Build the wrapper object and suffix for the route you selected before the next step, because the session helper and the binder both retain whatever bytes they are handed.
