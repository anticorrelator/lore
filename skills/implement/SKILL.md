...
3. **For each selected task in manifest dependency order, apply its edits directly** using the lead's `Read` / `Edit` / `Write` / `Bash` tools. Honor each phase's `**Verification:**` objectives **once, at that phase's close** — after its last selected task — not per task; per-task, run only tests covering the surface that task's edits touched. Read each task description the same way a worker would. **Reviewer-facing comment discipline applies to the lead too** — apply the drop/keep rules, drift test, and worked examples from `agents/worker.md` step 5 to any comments you write into committed source.
