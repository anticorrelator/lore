...
Role template versions default from the implement-skill/worker/advisor templates when the flags are omitted (warn-degrade to unstamped). The flags take one version per role for the run and are the historical fallback: on a compiled run pass the legacy values as before, and know that they never override a compiled candidate's producer, which the verb resolves from the candidate's source claims. The verb's own summary entry is filed under `--template-version`, the lead's hash. Exit codes: `0` batch processed (accepted/rejected lists on stdout); `1` usage error / unreadable or malformed candidates file / no match / summary-log failure; `2` ambiguous reference.
