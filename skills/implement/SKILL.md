...
Selection is the caller's declaration — exactly one mode is required, no default: `--all` (every task) or `--task <id>` (repeatable; e.g. `task-3`). Use subset selection when resuming or staging a plan across sessions.
