...
Read this file from `skills/renormalize/SKILL.md` Step 5 Wave 1. For each fixer and retry, run `lore dispatch guidance` immediately before launch; if it fails, do not launch that brief. Assemble the prompt from that invocation's complete guidance output verbatim, the block below with `$KDIR`, the work-item slug, and the assigned entry set injected, then the report contract. Wave 1 reports must be accepted before any Wave 2 dispatch.
