...
Read this file from `skills/renormalize/SKILL.md` Step 5 Wave 2. Four role briefs follow; the lead assembles batches from them after the ownership partition, injects `$KDIR`, the work-item slug, and the batch's action subset, appends the report contract, and dispatches. When roles are merged into one batch, run their sections in the order listed here.
