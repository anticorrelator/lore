...
**Routing floor (user directive; third occurrence 2026-08-11 — operator's words: "why are we firing up fable subagents? I thought our skill guidance explicitly notes that we should be using codex to save token cost").** Coordinated dispatch — including read-only research and verification fan-outs, not just fix work — defaults to codex worker sessions via `lore session request` with no model flags (resolver defaults carry the directive; full text: `preferences/in-coordinate-dispatches-this-user-model-routing-d.md`). A harness-native Agent spawn is the exception, and when used it carries `model: "opus"` explicitly on EVERY spawn — an omitted model override silently inherits the seat's model and bills the expensive tier (failed this way 2026-07-28 and 2026-08-11). Pre-spawn check, every time: *does this Agent call state a model?* If in-flight spawns are found in violation: grandfather them to finish, add one independent codex review pass over their output, ledger the correction.
