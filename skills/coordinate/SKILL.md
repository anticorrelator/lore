...
**Progress, completion, teardown, and worktree disposition are distinct facts.** `step_completed` marks a durable intra-protocol boundary (hosted `/spec`: investigation, accepted design, plan-ready; hosted `/implement`: one per task after full acceptance — worker completions, claims, consultations, and echoes never emit). `terminus_reached` says the protocol's durable writes finished; it does not say the isolated worktree was published. The `closed`/`close_failed`/`orphaned` family describes process teardown. Worktree disposition is exactly `published`, `restore_refused`, or `worktree_quarantined`: successful `published` projects to the normal `closed` row, while refusal/quarantine add their named recovery rows rather than a second close terminal.
...
- **Read the worktree disposition.** Treat the normal `closed` row as the `published` projection only when no linked refusal/quarantine row displaced it. On `restore_refused` or `worktree_quarantined`, verify the destination stayed byte-for-byte unchanged and record the durable result ref/patch for reconciliation. `teardown-pending` still owns the tree; quarantine preserves content durably but does not retain the physical directory forever.
