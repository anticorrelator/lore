...
**The generic dispatch** is `lore session start <item> --workspace <checkout> --framework <id> --model <model> --context <brief> --packet <id> --key <dispatch-id> --json`, returning a durable handle for `send`, `answer`, `peek`, `wait`, `inspect`, `attach`, `close`. `lore defaults` carries the routing directives, and the guidance floor rides every brief (`lore dispatch guidance` for harness-native spawns). A `--framework` override needs a live, ledgered reason for this dispatch.
