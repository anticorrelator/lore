...
`lore session events --since <cursor>`. The cursor rides stdout as a final `{"next_cursor": N}` row, alongside the event rows — read the whole stream, no stderr to fold back in. It is opaque: persist it verbatim in the ledger, so observation itself stays auditable. `--cursor-only` gets a baseline without replaying the journal. Interpret, don't re-validate: vocabulary and row shape are the sole writer's job. Match lifecycle pairs by per-slug ordering, never adjacency. `list` for the live snapshot, `peek` for judgment calls; phase boundaries inside a protocol session don't journal, so never promise an intervention window keyed to one. Scope any watcher's wake condition by slug *and* session_type — and a sloppy wake is tolerable only because the read must be correct: re-read before any ledger row. Rather than hand-roll a poll loop, block on `lore session wait <slug>` — it wakes on `closed`/`close_failed`/`orphaned` by default, keys on exact-slug so a worker's close never wakes a parent (`--work-item <slug>` opts in, matching the base slug plus its derived `--w<n>` worker slugs), and hands back a resume cursor on every exit (0 matched, 2 timed out — re-arm from that cursor, 3 session-gone, 4 internal error after bounded retries — never read 4 as timeout). `--request-id` narrows `closed` rows only; a slug-matched `close_failed` still wakes — sloppy wake, exact read (c2c34e2). `--until modal_blocked` is the opt-in wake for mid-work harness-modal stalls (one journal row per state entry, ebc500b); arm it alongside the terminal wait on manual-approval instances, where the modal is the likeliest stall. A mid-row `--since` is refused with `cursor-not-row-aligned` — cursors are copied verbatim from stream output, never computed. One closure the verb honestly refuses to absorb: a running watcher dies if a live tree-writer rewrites `session-wait.sh` or anything in its dependency closure mid-poll — check the writer's declared file set before arming, and drop to a raw journal-grep poll when it overlaps (n=2 across arcs, 2026-07-11). Capture the baseline *before* you act on a teardown you mean to measure: `--cursor-only`, then close, then `wait --since` that cursor. Send exits: `0` delivered, `3` mid-turn (back off, re-poll), `1` gone.
...
- close retry-on-unblock, or a `terminus_reached` event — n=1 (eval-harness arc, 2026-07-12): a spec session COMPLETED its protocol but both terminus auto-closes had already died against a modal (`close_failed` reason=interactive-prompt); nothing re-arms a close when the modal state exits, so the finished session parked open ~18 min until the human asked why the result hadn't arrived. Completion and closure are distinct; only closure journals — the wait verb faithfully watched a close that could never self-fire. Aggravator: peek's modal classifier reads codex's composer suggestion as `blocked_reason=modal` (evidence on [[work:session-verbs-cannot-answer-harness-native-modals]]). Remedy legs (either suffices) → [[work:completed-sessions-park-open-close-retry-on-unbloc]]; the terminus-event leg is the same gap as the never-emitted `step_completed` token above — fold them if built. Interim seat idiom: when a wait wakes on `close_failed` and a later peek shows protocol-terminus narration at a resting composer, close it yourself; don't re-arm and hope.
