...
Reading the journal: `close_requested` and `closed` are ordered but not adjacent (activity events keep emitting during teardown) — match the pair by ordering per slug. `lore session list` for the live snapshot; `peek` (where present) for prompt-reading judgment calls. Two observability holes the journal does not cover: phase transitions inside a protocol session emit nothing (the Phase 1→2 boundary is visible only via peek — never promise the user an intervention window keyed to a phase boundary you can't observe), and a same-slug spawn over a live session can evict it without a `closed` row. Scope any hand-rolled watcher's wake condition by slug *and* session_type — an unscoped grep false-wakes on sibling streams (two live drifts in two runs).
...
- `close_refused` journal event + tiered close authority (user-decided 2026-07-06: idle human session honors explicit CLI close; generating refuses with a journaled row; terminus hold-open unchanged) — the silent-refusal trap produced three no-op closes a coordinator believed had acted. Lands with the close-path audit item.
