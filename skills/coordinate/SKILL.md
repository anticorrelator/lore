...
**A watcher entry belongs in project-local settings.** Every session that reads a settings file fires the entry it holds — a shared file arms sessions with no relation to the seat the entry names, each spending an exit on it at every turn boundary — and a settings file holds exactly one watcher entry, so installing into a file another seat armed replaces that seat's entry. The install reports the owner and scope it displaced; read that report, because the displaced seat is unwatched until its entry is put back. Project-local settings arm exactly the sessions working this board and leave every other seat's file alone.
