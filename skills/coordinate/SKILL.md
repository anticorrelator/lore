...
**The generic dispatch** is `lore session start <item> --workspace <checkout> --context <brief> --packet <id> --key <dispatch-id> --json`, returning a durable handle for `send`, `answer`, `peek`, `wait`, `inspect`, `attach`, `close`. When the caller already holds a producer-selected canonical route, it forwards it with `--session-route`. The guidance floor rides every brief (`lore dispatch guidance` for harness-native spawns).
