...
Compose one brief per subsystem and dispatch it through the route Step 3 selected. Fill every `<...>` placeholder at composition time; the report id, work-item slug, dispatch path, harness name, and template version are dispatch-assigned, never chosen by the explorer. The first placeholder is the complete output of a successful, per-launch `lore dispatch guidance` call. Copy it verbatim; if rendering failed, there is no runnable brief.

```
<invocation-fresh dispatch guidance block>
