...
# --- Defaults ---
MAX_BUDGET="5.0"
# MODEL is empty by default; per-role resolution runs after arg parsing
# so an explicit --model flag takes precedence over the role map. The
# orchestrator agent runs as `lead` (drives /implement); the
...
Options:
  ...
  --model <model>      Override the model used for both the implement orchestrator
                       and the suitability judge. When omitted, the orchestrator
                       is resolved from role 'lead' and the judge from role 'judge'
...
# the judge role. JUDGE_MODEL_OVERRIDE captures the user's explicit
# override so both call sites honor it.
JUDGE_MODEL_OVERRIDE=""
if [[ -n "$MODEL" ]]; then
  JUDGE_MODEL_OVERRIDE="$MODEL"
fi
if [[ -z "$MODEL" ]]; then
  if ! MODEL=$(resolve_model_for_role lead implement 2>/dev/null) || [[ -z "$MODEL" ]]; then
    die "No model binding for role 'lead'. Pass --model <model> or set harnesses.<active>.ceremony_roles.implement.lead / harnesses.<active>.roles.lead in ~/.lore/config/settings.json."
  fi
fi
...
# Reads from CANDIDATE_SLUGS[]/CANDIDATE_TITLES[]/CANDIDATE_RISK[].
# Populates parallel arrays:
...
judge_candidates() {
  ...
  if ! command -v claude &>/dev/null; then
    echo "[batch-implement] Warning: claude CLI not found, skipping judge"
    for i in "${!CANDIDATE_SLUGS[@]}"; do
      CANDIDATE_VERDICT+=("—")
      CANDIDATE_JUDGE_REASON+=("")
    done
  ...
  echo "[batch-implement] Running LLM judge on ${#CANDIDATE_SLUGS[@]} candidates..."
...
risks = sys.argv[3].split(',') if sys.argv[3] else []

candidates = []
for i, slug in enumerate(slugs):
    plan_path = os.path.join(work_dir, slug, 'plan.md')
    plan = ''
    ...
    candidates.append({
        'slug': slug,
        'heuristic_risk': risks[i] if i < len(risks) else 'unknown',
        'plan_content': plan
    })

print(json.dumps(candidates, indent=2))
" "$WORK_DIR" "$(IFS=,; echo "${CANDIDATE_SLUGS[*]}")" "$(IFS=,; echo "${CANDIDATE_RISK[*]}")")
  ...
  # Build user prompt
  local prompt_file
  prompt_file=$(mktemp /tmp/batch-impl-judge-XXXXXX.md)
  cat > "$prompt_file" <<PROMPT
# Batch-Implement Candidate Evaluation

Evaluate each work item for suitability as an autonomous \`/implement\` target.
The orchestrator spawns worker agents that execute tasks from plan.md independently.
...
PROMPT
  ...
  # Call claude
  local output_file
  output_file=$(mktemp /tmp/batch-impl-judge-result-XXXXXX.txt)
  ...
  if [[ -z "$judge_model" ]]; then
    if ! judge_model=$(resolve_model_for_role judge 2>/dev/null) || [[ -z "$judge_model" ]]; then
      echo "[batch-implement] Warning: no model binding for role 'judge', skipping judge"
      for i in "${!CANDIDATE_SLUGS[@]}"; do
        CANDIDATE_VERDICT+=("—")
        CANDIDATE_JUDGE_REASON+=("")
      done
  ...
  if [[ "$judge_ok" == false ]]; then
    echo "[batch-implement] Warning: judge call failed, proceeding without filtering"
    rm -f "$output_file"
    for i in "${!CANDIDATE_SLUGS[@]}"; do
      CANDIDATE_VERDICT+=("—")
      CANDIDATE_JUDGE_REASON+=("")
...
" < "$output_file") || parse_ok=false
  ...
  if [[ "$parse_ok" == false ]]; then
    echo "[batch-implement] Warning: could not parse judge response, proceeding without filtering"
    for i in "${!CANDIDATE_SLUGS[@]}"; do
      CANDIDATE_VERDICT+=("—")
      CANDIDATE_JUDGE_REASON+=("")
    done
  ...
  echo "[batch-implement] Judge verdicts: $suitable suitable, $marginal marginal, $unsuitable unsuitable"

  # Filter unsuitable candidates
  if [[ $unsuitable -gt 0 ]]; then
    local new_slugs=() new_titles=() new_risk=() new_phases=() new_tasks=() new_files=() new_open_q=() new_updated=() new_verdicts=() new_reasons=()
    for i in "${!CANDIDATE_SLUGS[@]}"; do
    ...
    echo "[batch-implement] Filtered $unsuitable unsuitable candidate(s)"
  fi
}
...
present_batch() {
  ...
  local table_spec
  if [[ "$USE_JUDGE" == true ]]; then
    table_spec="SLUG:flex:100:left|RISK:fixed:6:left|JUDGE:fixed:10:left|PHASES:fixed:6:right|TASKS:fixed:5:right|FILES:fixed:5:right|UPDATED:fixed:10:left"
  else
    table_spec="SLUG:flex:100:left|RISK:fixed:6:left|PHASES:fixed:6:right|TASKS:fixed:5:right|FILES:fixed:5:right|UPDATED:fixed:10:left"
  fi
  ...
  for i in "${!CANDIDATE_SLUGS[@]}"; do
    ...
    if [[ "$USE_JUDGE" == true ]]; then
      table_rows+="${slug}|${risk}|${verdict}|${phases}|${tasks}|${files}|${updated}"$'\n'
    else
      table_rows+="${slug}|${risk}|${phases}|${tasks}|${files}|${updated}"$'\n'
    fi
  done
  printf '%s' "$table_rows" | render_table "$table_spec"
...
# Writes JSON to _batch-runs/implement-<timestamp>.json.
report_results() {
  ...
  for i in "${!RESULT_SLUGS[@]}"; do
    ...
    case "$verify" in
      ...
      partial)
        ...
        printf "  %-${path_width}s (partial -- %s/%s tasks)\n" "_work/$slug/" "$checked" "$total_tasks"
        ;;
      *)
