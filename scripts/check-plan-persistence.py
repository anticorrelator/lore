#!/usr/bin/env python3
"""Stop hook: detect plan mode usage without persistence to _plans/.

Reads the session transcript, checks for ExitPlanMode tool uses, and verifies
that the plan was persisted to the knowledge store's _plans/ directory.
...
def main():
    ...
    try:
        result = subprocess.run(
            ["bash", os.path.join(script_dir, "resolve-repo.sh")],
            capture_output=True,
            text=True,
            timeout=5,
        )
        # First line of stdout is the path; warnings go to stderr
        knowledge_dir = result.stdout.strip().split("\n")[0].strip()
    except Exception:
        sys.exit(0)
        ...
        json.dump(
            {
                ...
                "reason": (
                    ...
                    "ephemeral. Please persist the plan to `_plans/` using `/explore-and-plan create`."
                ),
            },
            sys.stdout,
        )
    ...
    plans_dir = os.path.join(knowledge_dir, "_plans")

    # Check if any _plans/ subdirectory has files modified after the ExitPlanMode
    ...
    json.dump(
        {
            ...
            "reason": (
                ...
                "persisted to `_plans/`. The builtin plan file at `~/.claude/plans/` is ephemeral "
                "and will be lost across sessions.\n\n"
                "Persist the plan now:\n"
                "1. Use `/explore-and-plan create` to persist via the skill, OR\n"
                "2. Manually create `_plans/<slug>/` with `_meta.json` and `plan.md`\n\n"
