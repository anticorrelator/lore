...
Used by:
- pk_search.py Resolver: backlink resolution with #heading fragments
- load-knowledge.sh: v1 (monolithic) format context-aware loading (legacy path)

For format_version 2 (file-per-entry), section extraction is rarely needed since
each entry is its own file. However, multi-section files (work plans, domain docs)
still benefit from heading-based extraction via the Resolver.
