...
# The git diff is the account of what shipped, on every path. A coordinated
# stream's own account is its integration commit and the suite counts beside it
# in the arc ledger — neither of which this render restates.
...
diff_cov = coverage("present" if diff_rows else "present-empty", None)
