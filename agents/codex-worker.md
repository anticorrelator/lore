...
1. Call `TaskList` to see available tasks; claim your assigned one with `TaskUpdate` (`owner` = your name, `status` = in_progress). On the investigator shape the spec lead may have created no team task for this relay; then there is nothing to claim, and the report goes back as your message alone.
...
**Compiled path (`{{payload_file}}` set).** The prompt already exists. It is the frozen payload at `{{payload_file}}`: the guidance floor the lead rendered, the packet pointer, the identity header lines, the compiled worker or investigator brief, the JSON identity envelope, and this file's chaperone note (the last section of this document). Use it as `PROMPT_FILE` exactly as it is:
...
Read `$REPORT_FILE` — Codex's final message, captured by `-o`. On the worker shape it is a valid report only if it contains a `Task:` or `**Task:**` label and the `**Changes:**`, `**Observations:**`, and `**Tier 2 evidence:**` labels. If the run has a **non-zero exit, an empty or missing report file, or missing labels**, skip 6.3 entirely — do NOT append evidence from a degraded run — and go straight to the degraded template in 6.4. A degraded run's rows are untrustworthy; appending them would poison the evidence trail.
...
On the investigator shape (`{{report_shape}}` is `investigator`) the worker labels are the wrong test: an investigation answers a question, and gating its return on `**Changes:**` would degrade every honest investigator report. Check instead that the file is non-empty and carries the bold labels the investigator contract names — Question, Findings, Key files, Implications, Assertions, Observations, Worker leads, Unknowns:
...
**Compiled path, investigator shape.** Relay the body to the lead who dispatched you and stop. The spec collector lands it at the assigned destination, appends the assertions, and runs the completion check; a copy landed here would make the collector's write-once landing refuse. Complete a team task only if the lead assigned you one.
...
The shared knowledge store is outside your sandbox, so `evidence-append.sh` and `lore coordinate report` fail here with `Operation not permitted`. Do not run them. Build each Tier 2 row as the claim forms, with `task_id` set to `position_dispatch.bindings.task_id` and `producer_role` `worker`; `python3 ~/.lore/scripts/snippet_normalize.py --hash` only hashes text and runs in-sandbox. Collect the rows and print them after **Blockers:** inside this exact block, one compact single-line JSON row per line, both sentinel lines present even for one row, nothing else between them:

  ===LORE-TIER2-BEGIN===
  {compact single-line JSON row}
  ===LORE-TIER2-END===

The chaperone appends each row from outside the sandbox and replaces your **Tier 2 evidence:** body with the claim IDs that landed. If you formed no claims, write `none` there and omit the block. Close criteria run through `lore criteria run <slug> <task-id> <criterion-id> --execution-worktree "$PWD" --packet-id <packet-id>`; cite result IDs in Checks.
