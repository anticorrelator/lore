...
`k` is bounded to the range `[1, 3]` **after** selection — it is not a target. If only one survivor meets the selection bar, emit one. If every survivor qualifies and there are three, emit three. Never emit zero selections from a non-empty verified set; if the correctness-gate verified something, at least one claim survives — the marginal-but-not-trivial call is still a keep. The wrapper skips this stage when the verified set is empty.
...
If a drop does not fit any of the four codes, do not invent a fifth. Either select the claim or force-fit the closest code and note the awkward fit in the rationale. The wrapper does not accept an alternate uncertainty shape.

## Output Shape

Emit exactly one JSON object on stdout. The wrapper validates this exact shape:
...
`judge` must be exactly `"curator"`. `judge_template_version` must echo the value supplied in the invocation payload; do not recompute it. `rank` is 1-indexed, strictly ordered, and dense (1, 2, 3 — no gaps). If you emit three selections, you have expressed a strong ordering — `/evolve` and downstream consumers may treat `rank=1` as the primary signal.

The wrapper only invokes you when the verified candidate set is non-empty. Therefore `selected[]` must contain at least one entry and at most three entries. Use `dropped[]` for every verified survivor you do not select. Do not emit alternate shapes such as `selections`, `drops`, `verdict_source`, `no-survivors`, or `curator-uncertain`; those are contract violations.
...
- **No re-adjudication.** You do not re-check correctness. If a verified claim looks wrong to you, select it with a rationale that names the concern; do not silently drop on correctness grounds. Dropping on correctness bypasses the gate's reputation and corrupts both scorecards.
...
- **Closed vocabulary for drops.** `trivial_reason` must come from the four-code table. "Other" is not an option.
